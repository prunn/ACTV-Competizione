...
import threading
import random
from .util.classes import Window, Label, Value, POINT, Colors, Log, raceGaps, Font, Laps, MyHTMLParser, Config, HTMLParserPing
...
class ACTower:
    ...
    def redraw_size(self):
        ...
        if self.ui_row_height.hasChanged() or self.font.hasChanged():
            ...
            self.lbl_title_mode.set(w=self.ui_row_height.value * 170/38, h=height - 2, y=-(height - 2))
            self.lbl_title_mode_txt.set(w=self.ui_row_height.value * 170/38, h=height - 2,
            ...
            for driver in self.drivers:
                ...
                if len(p) > 0  and p[0] < self.max_num_cars + display_offset and (p[0]==0 or p[0] > display_offset):
                    ...
                    if c > 0:# and driver.completedLaps.value > self.minLapCount
                        ...
                        driver.set_time(0, self.standings[0][1], self.sessionTimeLeft, self.qual_mode.value)
                    driver.update_pit(self.sessionTimeLeft)
                else:
                    driver.hide()
        ...
        if sim_info_status == 2:# or (sim_info_status == 1 and self.session.value < 2):
            ...
            if self.session.value < 2:
                ...
                for driver in self.drivers:
                    ...
                    #bl = c + spline
                    #if self.sector_is_valid(bl, driver):
                    #    driver.race_current_sector.setValue(math.floor(bl * 100))
                ...
                if not math.isinf(self.sessionTimeLeft):
                    ...
                    if self.TimeLeftUpdate.hasChanged():
                        ...
                        #t_update_drivers_end = time.time() and  self.drivers[i].isInPit.value
